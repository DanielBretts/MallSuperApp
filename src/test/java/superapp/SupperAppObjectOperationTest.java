...
public class SupperAppObjectOperationTest {
	...
	public void putExistingChildToObject() throws Exception{
		...
		ObjectBoundary[] actualChildersns = this.restTemplate
				.getForObject(this.baseUrl + "/superapp/objects/{superapp}/{InternalObjectId}/children"
						, ObjectBoundary[].class, superapp ,o1);
		assertThat(actualChildersns)
				.usingRecursiveFieldByFieldElementComparatorOnFields("ObjectId.internalObjectId")
				.contains(o2Boundary);
	}
	@Test
	public void testObjectCanBeRelatedToManyObjects() throws Exception{
		...
		objectBoundary0.setObjectId(this.restTemplate.postForObject
				(this.baseUrl + "/superapp/objects", objectBoundary0, ObjectBoundary.class).getObjectId());
		String ob0 = objectBoundary0.getObjectId().getInternalObjectId();
		
		objectBoundary1.setObjectId(this.restTemplate.postForObject
				(this.baseUrl + "/superapp/objects", objectBoundary1, ObjectBoundary.class).getObjectId());
		...
		ObjectBoundary[] actualchildrens = 
				this.restTemplate
				.getForObject(this.baseUrl + "/superapp/objects/{superapp}/{InternalObjectId}/children",
						ObjectBoundary[].class, superapp, ob0);
