...
public class MiniAppsTests {
	...
	public void teardown() {
		this.restTemplate
			.delete(this.baseUrl + "/superapp/admin/miniapp");
	}
	...
	public void testSuccessfulCreateUser() throws Exception{
		...
		MiniAppCommandBoundary miniAppCommandBoundary = new MiniAppCommandBoundary("test",new TargetObjectBoundary(new ObjectId("123")),
				new InvokedBy(new UserId("daniel@mail.com")),commandAttributes);
		CommandID commandID = this.restTemplate.postForObject(this.baseUrl + "/superapp/miniapp/test", miniAppCommandBoundary, MiniAppCommandBoundary.class).getCommandId();
		miniAppCommandBoundary.setCommandId(commandID);
		
		// THEN the miniappCommand is stored to the database
		MiniAppCommandBoundary miniAppCommandFromDb = this.restTemplate.getForObject(this.baseUrl + "/superapp/admin/miniapp/test", MiniAppCommandBoundary[].class)[0];	
		assertThat(miniAppCommandFromDb)
			.isNotNull()
