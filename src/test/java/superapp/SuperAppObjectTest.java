...
//										//		}
		...
		ObjectBoundary ObjectFromDatabase = this.restTemplate
				.getForObject(this.baseUrl + "/superapp/objects", ObjectBoundary[].class)[0]; // get message using GET HTTP METHOD
				
		assertThat(ObjectFromDatabase)
				.isNotNull()
				.extracting("ObjectId.internalObjectId", "type","alias","active","location.lat","location.lng","createdBy.userId.email")
//				.isEqualTo(newMessage.getMessage());
				.containsExactly(
						newObjectBoundary.getObjectId().getInternalObjectId(),
						newObjectBoundary.getType(),
						newObjectBoundary.getAlias(),
						newObjectBoundary.getActive(),
						newObjectBoundary.getLocation().getLat(),
...
//		// AND the database is empty
		...
		objectBoundary.setObjectId(
				this.restTemplate
				.postForObject(this.baseUrl+"/superapp/objects", objectBoundary, ObjectBoundary.class)
				.getObjectId());
		
		objectBoundary.setAlias("braik");
		// WHEN I PUT /superapp/objects with a new objectBoundary 
		this.restTemplate.put(this.baseUrl + "/superapp/objects/{superApp}/{id}", objectBoundary,
				"2023b.shir.zur"
		...
		ObjectBoundary actualOrigin = this.restTemplate
				.getForObject(this.baseUrl + "/superapp/objects", ObjectBoundary[].class)[0];
		assertThat(actualOrigin)
				.isNotNull()
				.extracting("ObjectId.internalObjectId","alias")
				.containsExactly(objectBoundary.getObjectId().getInternalObjectId()
	...
	public void testSuccessfulGetSpecipicObject() {
		...
		objectBoundary.setObjectId(this.restTemplate.postForObject(this.baseUrl + "/superapp/objects", objectBoundary, ObjectBoundary.class).getObjectId());
		
		// THEN I GET THIS OBJECT // /superapp/objects/{superApp}/{id}
		ObjectBoundary objectBoundary1 = this.restTemplate.getForObject(this.baseUrl + "/superapp/objects/{superApp}/{id}", ObjectBoundary.class, "2023b.shir.zur", objectBoundary.getObjectId().getInternalObjectId());
	...
	public void testSuccessfulGetAllObject() {
		...
		// WHEN I GET THIS POST // /superapp/objects
		objectBoundary.setObjectId(this.restTemplate.postForObject(this.baseUrl + "/superapp/objects", objectBoundary, ObjectBoundary.class).getObjectId());
		objectBoundary0.setObjectId(this.restTemplate.postForObject(this.baseUrl + "/superapp/objects", objectBoundary0, ObjectBoundary.class).getObjectId());

		// THEN I GET THIS ALL OBJECTS // /superapp/objects		
		ObjectBoundary[] objectBoundaryFromDB = this.restTemplate.getForObject(this.baseUrl + "/superapp/objects", ObjectBoundary[].class);
		assertThat(objectBoundaryFromDB).isNotNull();
		assertThat(objectBoundaryFromDB[0]).isNotNull().extracting("ObjectId.internalObjectId", "type", "alias").containsExactly(objectBoundary.getObjectId().getInternalObjectId(), objectBoundary.getType(), objectBoundary.getAlias());
		assertThat(objectBoundaryFromDB[1]).isNotNull().extracting("ObjectId.internalObjectId", "type", "alias").containsExactly(objectBoundary0.getObjectId().getInternalObjectId(), objectBoundary0.getType(), objectBoundary0.getAlias());
